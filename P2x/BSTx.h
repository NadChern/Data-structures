...
// contains, remove, empty, size, getLeafCount, getHeight, getWidth, getLevel,
// getInOrderTraversal, getPreOrderTraversal, getPostOrderTraversal,
// getLevelOrder,  getAncestors.
...
public:
    ...
    // IN: none
    // MODIFY: none
    // OUT: return the width of the tree(largest number of nodes
    // in the same level)

    string getLevelOrder() const;
    // Traverse elements of BST level by level
    // IN: none
    // MODIFY: none
    // OUT: return a string that results from traversing the BST level by level
...
private:
    ...
    // Helper function for public getWidth function
    // IN: pointer to the root(current node), level
    // MODIFY: none
    // OUT: return the width of the level
...
int BST<T>::getWidth() const {
    int maxWidth = 0; // hold max width value when traversing levels
    int height = getHeight(); // hold height of the tree
    int width; // hold width of each level
...
template<typename T>
int BST<T>::getWidth(Node *ptr, int level) const {
    if (ptr == nullptr) {
        return 0;
    }
    if (level == 0) {
...
template<typename T>
string BST<T>::getLevelOrder() const {
    stringstream ss; // creat stream object
    if (empty()) {
        return "";
    }

    queue<Node *> queue; // create queue object of nodes
    queue.push(root); // push root node into the queue to start traversal

    while (!queue.empty()) { // till all levels checked
        Node *ptr = queue.front(); // assign front element of the queue to node
        queue.pop(); // remove front element from the queue
        ss << ptr->data << " ";
        if (ptr->left != nullptr) { // if ptr has left child
            queue.push(ptr->left); // it's pushed into the queue
        }
